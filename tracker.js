...
(function () {
  ...
  function render() {
    info.textContent = `Spin: ${sessionSpin} | ${getNextTarget()} | Mode: ${mode}`;
  ...
  makeBtn("🐢 Normal", function () {
    mode = mode === "normal" ? "instant" : "normal";
    this.textContent = mode === "normal" ? "🐢 Normal" : "⚡ Instant";
    render();
  });

  let lastStableText = "", lastStableNum = 0, lastSpinTime = 0;
  const STABLE_DELAY = 100;
  const COOLDOWN_NORMAL = 800;
  const COOLDOWN_INSTANT = 200;
  ...
  function detectHybridLoop() {
    ...
    const cooldown = mode === "instant" ? COOLDOWN_INSTANT : COOLDOWN_NORMAL;
    if (txt) {
      ...
      if (txt !== lastSeenText) {
        ...
        stableTimer = setTimeout(() => {
          if (txt === getMultiplierText()) {
            ...
            const passedDelay = timeSinceLastSpin >= cooldown;
